...
- **[ADR-012] Dual-Layer Persistence & Isolated Org Filtering**: `+page.ts` server load and `profileStore` / `organizations` state employ dual persistence: querying Supabase cloud database (`organizations`, `profiles`, `organization_members`) AND caching/merging with `localStorage` (`txtgrph_local_organizations`, `txtgrph_user_profile`). `workspaceStore.getChildFolders(parentId, orgId)` and `workspaceStore.getFolderDiagrams(folderId, orgId)` enforce strict explicit `orgId` filtering (`orgId = null` for Personal section, `orgId = org.id` for Team Spaces), completely isolating Team Space items from Personal files.
...
- **[Gotcha] Supabase RLS Insert `WITH CHECK (auth.uid() = owner_id)`**: Inserting into `public.organizations` fails under Supabase RLS unless `owner_id: userId` is explicitly specified in the insert payload and an initial `role: 'owner'` row is inserted into `public.organization_members`.
- **[Gotcha] Svelte 5 Snippet Parameter Scope Overriding**: In Svelte 5 snippets (`{#snippet folderTreeItem(folder, depth, orgId)}`), helper functions like `workspaceStore.getChildFolders(folder.id)` that depend on internal store state (`this.activeOrgId`) will default to active selection unless explicit parameters (`orgId`) are passed down through recursive snippet invocations.