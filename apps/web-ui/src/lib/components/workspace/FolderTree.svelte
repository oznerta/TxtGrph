...
{#snippet folderTreeItem(folder: Folder, depth: number)}
  {@const isExpanded = workspaceStore.expandedFolderIds.has(folder.id)}
  {@const subFolders = workspaceStore.getChildFolders(folder.id)}
  {@const diagrams = workspaceStore.getFolderDiagrams(folder.id)}
  ...
  {#if isExpanded}
    <div class="pl-3.5 space-y-0.5 border-l border-white/10 ml-2.5 my-0.5">
      ...
      {#each subFolders as childFolder (childFolder.id)}
        {@render folderTreeItem(childFolder, depth + 1)}
      {/each}
...
<div class="flex flex-col h-full bg-[#000000] border-r border-white/10 select-none text-white/80 font-['Instrument_Sans',sans-serif]">
  ...
  <div class="flex-1 min-h-0 overflow-y-auto px-3 py-4 space-y-6 text-[13px] custom-scrollbar">
    ...
    <div class="space-y-1">
      ...
      <!-- Dashboard Root Link -->
      <button
        onclick={() => workspaceStore.selectFolder(null)}
        class="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl transition-all font-semibold {workspaceStore.activeDiagramId === null && workspaceStore.activeFolderId === null ? 'bg-white/15 text-white border border-white/20 shadow-sm' : 'text-white/60 hover:bg-white/10 hover:text-white'}"
      >
        <LayoutDashboard size={16} class={workspaceStore.activeDiagramId === null && workspaceStore.activeFolderId === null ? 'text-white' : 'text-white/50'} />
        <span>Dashboard</span>
      </button>
        ...
        {#if personalExpanded}
          <div class="pl-4 space-y-0.5 border-l border-white/10 ml-5 my-1">
            ...
            {#each workspaceStore.getChildFolders(null) as folder (folder.id)}
              {@render folderTreeItem(folder, 1)}
            {/each}
    ...
    <div class="space-y-2 pt-2 border-t border-white/10">
      ...
      {#each organizations as org (org.id)}
        <button
          onclick={() => onOpenOrgSettings(org.id, org.name)}
          class="w-full flex items-center justify-between px-3 py-1.5 rounded-xl text-white/90 hover:bg-white/10 transition-colors truncate text-xs font-semibold cursor-pointer group"
        >
          <div class="flex items-center gap-2 truncate">
            <Building2 size={14} class="text-amber-400/80 shrink-0" />
            <span class="truncate">{org.name}</span>
          </div>
          <Settings size={12} class="text-white/30 group-hover:text-white transition-colors shrink-0" />
        </button>
      {/each}
  ...
  <div class="p-3 border-t border-white/10 shrink-0">
    ...
    >
      <div class="flex items-center gap-3 min-w-0">
        <div class="w-8 h-8 rounded-full bg-white text-black font-bold flex items-center justify-center text-xs shrink-0 shadow-md">
          {initial}
        </div>
        <div class="min-w-0 flex-1">
          <p class="text-xs font-bold text-white truncate group-hover:text-white transition-colors leading-tight">
            {userEmail || 'User Account'}
          </p>
          <span class="text-[10.5px] text-white/60 block font-medium font-['IBM_Plex_Mono',monospace] mt-0.5">Free & Open Source</span>
        </div>
      </div>
